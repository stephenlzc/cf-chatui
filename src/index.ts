...
const SUPPORTED_MODELS = {
  chat: [
    { id: '@cf/zai-org/glm-4.7-flash', name: 'GLM-4.7 Flash', description: '高性能对话模型' },
    { id: '@cf/openai/gpt-oss-120b', name: 'GPT-OSS 120B', description: '开源大语言模型' },
  ],
  image: [
    { id: '@cf/black-forest-labs/flux-2-dev', name: 'FLUX.2 Dev', description: '高质量图像生成' },
  ],
  embedding: [
    { id: '@cf/pfnet/plamo-embedding-1b', name: 'Plamo Embedding 1B', description: '文本嵌入模型' },
  ],
};
...
async function handleLogin(request: Request, env: Env): Promise<Response> {
  try {
    ...
    return createJSONResponse({ token, message: '登录成功' });
  } catch {
...
async function handleChat(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    // GLM模型在非流式模式下返回最终答案，流式模式下返回思考过程
    // 所以GLM模型不使用stream模式
...
async function handleGenerateImage(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (!body.prompt) {
      return createErrorResponse('请提供提示词', 400);
    }
    ...
    if (!aiResponse.ok) {
      ...
      return createErrorResponse('图片生成失败: ' + errorData, 500);
    }
...
async function handleEmbeddings(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (!aiResponse.ok) {
      ...
      return createErrorResponse('嵌入生成失败', 500);
    }
